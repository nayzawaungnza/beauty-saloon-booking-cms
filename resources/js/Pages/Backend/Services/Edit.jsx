...
const Edit = ({ service, errors = {} }) => {
  document.title = `Edit ${service.name} | Admin Panel`

  const editorRef = useRef(null)
  const [selectedFiles, setSelectedFiles] = useState([])
  const [selectedImage, setSelectedImage] = useState(null)
  const [galleryImages, setGalleryImages] = useState([])
  const [editorContent, setEditorContent] = useState(service.description || "")
  const [existingImages, setExistingImages] = useState({
    default: service.default_image || null,
  ...
  const { data, setData, post, processing } = useForm({
    ...
    price: service.price || "", // Keep as is since your backend stores it directly
    is_promotion: service.is_promotion || false,
    discount_price: service.discount_price || "",
    requires_buffer: service.requires_buffer || false,
  ...
  // Enhanced CKEditor configuration (same as Create.jsx)
  ...
  useEffect(() => {
    if (service.default_image) {
      setSelectedImage(`/storage/${service.default_image.image_url}`)
    }
  ...
  const handleGalleryChange = (event) => {
    ...
    const totalExistingImages = existingImages.gallery.length - imagesToDelete.length
    const totalNewImages = galleryImages.length + files.length

    if (totalExistingImages + totalNewImages > 10) {
  ...
  const removeExistingGalleryImage = (imageId) => {
    console.log("Marking gallery image for deletion:", imageId)
    setImagesToDelete((prev) => {
      const newList = [...prev, imageId]
      console.log("Updated delete list:", newList)
      return newList
    })
  ...
  const removeMainImage = () => {
    ...
    if (existingImages.default) {
      console.log("Marking default image for deletion:", existingImages.default.id)
      setImagesToDelete((prev) => {
        const newList = [...prev, existingImages.default.id]
        console.log("Updated delete list after removing default:", newList)
        return newList
      })
      setExistingImages((prev) => ({ ...prev, default: null }))
    }
  ...
  // Replace the handleSubmit function with this simpler version
  const handleSubmit = (e) => {
    ...
    // Add images to delete - this is the important part
    if (imagesToDelete.length > 0) {
      console.log("Images to delete:", imagesToDelete) // Debug log
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="Services"
            breadcrumbItem="Edit Service"
            breadcrumbItems={[
              { title: "Services", link: route("admin.services.index") },
              { title: service.name, link: route("admin.services.show", service.id) },
            ...
            <Row>
              <Col lg={8}>
                <Card>
                  <CardBody>
                    ...
                    <div className="mb-3">
                      <Label htmlFor="excerpt-input">Service Excerpt</Label>
                      <Input
                        id="excerpt-input"
                        name="excerpt"
                        type="textarea"
                      ...
                      <small className="text-muted">A brief summary that will be displayed in service listings.</small>
                    </div>
                    ...
                    <Row>
                      ...
                      <Col md={6}>
                        <div className="mb-3">
                          <Label htmlFor="discount-price-input">Discount Price</Label>
                          <div className="input-group">
                            <span className="input-group-text">$</span>
                            <Input
                    ...
                    {existingImages.gallery.length > 0 && (
                      <div className="mb-3">
                        ...
                        <div className="border rounded p-3">
                          <Row>
                            {existingImages.gallery.map((image) => (
                    ...
                    <div className="mb-3">
                      <Label className="form-label">Add New Gallery Images (Max 10 total)</Label>
                      <div className="border rounded p-3">
                        <div className="text-center">
                          ...
                          <p className="text-muted mt-2 mb-0">
                            Select multiple images for the service gallery. Maximum 10 images total, 2MB each.
                          </p>
                        </div>

                        {galleryImages.length > 0 && (
                          <div className="mt-3">
                            <h6 className="mb-3">New Images Preview ({galleryImages.length})</h6>
                            <Row>
                  ...
                  <CardBody>
                    ...
                    <div className="d-flex align-items-center mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-muted mb-0">{existingImages.gallery.length + galleryImages.length} images</p>
                      </div>
                    </div>
                    ...
                    <div className="d-flex align-items-center mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-muted mb-0">
                          {data.excerpt ? `${data.excerpt.substring(0, 30)}...` : "Not set"}
                        </p>
                      </div>
                    </div>
                    ...
                    {data.is_promotion && (
                      <div className="d-flex align-items-center mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted mb-0">
                            {data.discount_price ? `$${data.discount_price}` : "No discount price"}
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <Col lg={12}>
                ...
                {imagesToDelete.length > 0 && (
                  <div className="mb-3">
                    <Label className="form-label text-danger">Images marked for deletion (Debug)</Label>
                    <div className="border rounded p-2 bg-light">
                      <small>IDs: {imagesToDelete.join(", ")}</small>
                    </div>
                  </div>
                )}

                <div className="text-end mb-4">
                  <Link href={route("admin.services.show", service.id)} className="btn btn-secondary me-2">
                    Cancel
                  </Link>
